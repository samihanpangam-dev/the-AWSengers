...
class CedarSecurityEngine:
    ...
    def evaluate(self, principal: str, action: str, resource: str) -> CedarDecision:
        ...
        norm_action = action.replace('Action::"', '').replace('"', '').replace('Action::', '').strip()
        ...
        if self._has_native_cedar:
            try:
                ...
                req = {
                    "principal": principal,
                    "action": action if action.startswith("Action::") else f'Action::"{action}"',
                    "resource": resource,
                    "context": {},
                }
        ...
        # Cedar syntax: (forbid|permit) ( principal, action == Action::"...", resource );
        policies = re.findall(
        ...
        for effect, p_spec, a_name, r_spec in policies:
            if a_name.lower() == norm_action.lower():
